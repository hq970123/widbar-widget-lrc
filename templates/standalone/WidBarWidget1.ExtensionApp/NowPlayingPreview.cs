...
internal sealed class NowPlayingPreview : Grid
{
    ...
    public NowPlayingPreview(double lyricFontSize)
    {
        ...
        var controls = new StackPanel { Orientation = Orientation.Horizontal, Spacing = 2, VerticalAlignment = VerticalAlignment.Center, Margin = new Thickness(2, 0, 0, 0) };
        ...
        controls.Children.Add(_previous); controls.Children.Add(_playPause); controls.Children.Add(_next);
    ...
    public void Update(string title, string artist, string lyric, bool isPlaying, bool mediaAvailable, double lyricFontSize)
    {
        ...
        if (!string.Equals(_lastLyric, lyric, StringComparison.Ordinal)) { _lastLyric = lyric; _lyric.Text = lyric; AnimateLyricChange(); }
