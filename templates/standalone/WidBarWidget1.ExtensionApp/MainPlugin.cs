...
public sealed class MainPlugin : WidgetPluginBase, IConfigurableWidgetPlugin, IWidgetFlyoutLifecycle
{
    ...
    private TimeSpan PlaybackPosition => !_settings.UseWindowsMediaSession || !_mediaReady ? DateTime.Now - _startedAt : _playbackStatus == GlobalSystemMediaTransportControlsSessionPlaybackStatus.Playing ? _mediaPosition + (DateTime.Now - _mediaPositionReadAt) : _mediaPosition;
    ...
    public override UIElement? CreateFlyoutContent()
    {
        ...
        var panel = new StackPanel { Spacing = 13, Padding = new Thickness(24), VerticalAlignment = VerticalAlignment.Center }; panel.Children.Add(_titleText); panel.Children.Add(_statusText); panel.Children.Add(_previousText); panel.Children.Add(_currentText); panel.Children.Add(_nextText); RefreshLyrics();
    ...
    public UIElement? CreateSettingsContent(IWidgetSettingsContext context)
    {
        ...
        var title = new TextBox { Header = "Fallback song title", Text = draft.SongTitle }; var lrc = new TextBox { Header = "Fallback / manual LRC lyrics", Text = draft.LrcText, AcceptsReturn = true, TextWrapping = TextWrapping.Wrap, MinHeight = 150 }; var font = new Slider { Header = "Taskbar lyric font size", Minimum = 11, Maximum = 24, Value = draft.FontSize, StepFrequency = 1 };
        void Save() { draft.UseWindowsMediaSession = media.IsOn; draft.AutoFetchLyrics = auto.IsOn; draft.SongTitle = title.Text; draft.LrcText = lrc.Text; draft.FontSize = font.Value; context.SaveSettings(draft.ToJson()); context.RequestPreviewRefresh(); }
        media.Toggled += (_, _) => Save(); auto.Toggled += (_, _) => Save(); title.TextChanged += (_, _) => Save(); lrc.TextChanged += (_, _) => Save(); font.ValueChanged += (_, _) => Save();
        var panel = new StackPanel { Spacing = 16 }; panel.Children.Add(media); panel.Children.Add(auto); panel.Children.Add(title); panel.Children.Add(lrc); panel.Children.Add(font); return panel;
    }
    ...
    private (string previous, string current, string next) CurrentLyric()
    {
        ...
        var index = _lyrics.FindLastIndex(x => x.Time <= PlaybackPosition); if (index < 0) return ("", _lyrics[0].Text, _lyrics.Count > 1 ? _lyrics[1].Text : ""); return (index > 0 ? _lyrics[index - 1].Text : "", _lyrics[index].Text, index + 1 < _lyrics.Count ? _lyrics[index + 1].Text : "");
    }
    ...
    private void RefreshLyrics()
    {
        ...
        if (_statusText is not null) _statusText.Text = _loadingLyrics ? "Searching synced lyrics…" : _settings.UseWindowsMediaSession ? (_mediaReady ? $"{(_playbackStatus == GlobalSystemMediaTransportControlsSessionPlaybackStatus.Playing ? "Playing" : "Paused")}  {PlaybackPosition:mm\\:ss}" : "Waiting for Windows media…") : $"Manual timer  {PlaybackPosition:mm\\:ss}";
    }
